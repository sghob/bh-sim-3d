...
double disk_pot(double r, double z) {
    ...
    double dk = (4.0 / (0.5 * pc_m)) / n1;
    double dr = 2.5 * R_gd / n2;
    double cumsum = 0.0;
    ...
    //double omega_sq = pow(3 / (1E9 * yr_s), 2);
    //return 0.5 * omega_sq * z * z;
...
double fr(double r, double rdot, double thetadot, double z, double zdot, double cs) {
    ...
    return (1.0 / (M2 * sl_kg)) * (M2 * sl_kg * g_fr(r,z) + (M2 * sl_kg) * r * pow(thetadot, 2) + gasDFr(r, rdot, thetadot, z, cs, vg * vc(r)));

    //return (1.0 / (M2 * sl_kg)) * ((-1.0 * G * M1 * M2 * pow(sl_kg, 2)) / (pow(r , 2.0)) + (M2 * sl_kg) * r * pow(thetadot, 2) + gasDFr(r, r * thetadot, cs));

}

double ftheta(double r, double rdot, double theta, double thetadot, double z, double cs) {
    
    double v = pow(pow(rdot, 2.0) + pow(r * thetadot, 2.0), 0.5);
    //printf("\n%E", (1.0 / (r * M2 * sl_kg)) * (gasDFt(r, r * thetadot, cs) + (-2.0) * rdot * thetadot * M2 * (sl_kg)));
    //printf("\n%E", gasDFt(r, v, cs));
    return (1.0 / (r * M2 * sl_kg)) * ((-2.0) * rdot * thetadot * M2 * (sl_kg) + gasDFt(r, rdot, thetadot, z, cs, vg * vc(r))); //coriolis term to conserve angular momentum
}
...
State RK4Solver(State initial_state, double dt, double total_time) {
    ...
    while (t < total_time && current_state.r > 10.0 * pc_m && current_state.r < 1.0E4 * pc_m) {
        ...
        double k1t = thetadot;
        double k1td = ftheta(r, rdot, theta, thetadot, z, cs);
        double k1z = zdot;
        ...
        double k2td = ftheta(r + 0.5 * k1r * dt, rdot + 0.5 * k1rd * dt, theta + 0.5 * k1t * dt, thetadot + 0.5 * k1td * dt, z + 0.5 * k1z * dt, cs);
        ...
        double k3td = ftheta(r + 0.5 * k2r * dt, rdot + 0.5 * k2rd * dt, theta + 0.5 * k2t * dt, thetadot + 0.5 * k2td * dt, z + 0.5 * k2z * dt, cs);
        ...
        double k4td = ftheta(r + k3r * dt, rdot + k3rd * dt, theta + k3t * dt, thetadot + k3td * dt, z + k3z * dt, cs);
...
int main() {
    ...
    double rd0 = 0.0;
    double t0 = 0.0;
    double z0 = 500.0 * pc_m; //standard of 0.5 pc
    double zd0 = 0.0 * cs0;
    ...
    */
    ...
    relax(N, L, a, b, 50000, 4 * M_PI * G * rho0 * 1E-5 * h * h);
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            rho_flat[indexr++] = rho[i][j];
            phi_flat[index++] = phi[i][j];
        }
    }
